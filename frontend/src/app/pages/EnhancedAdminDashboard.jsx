...
export default function EnhancedAdminDashboard() {
  ...
  useEffect(() => {
    adminApi.getProductCategories().then(setProductCategories);
  }, []);
  ...
  const stats = [
    { label: "Total Projects", value: safeProjects.length },
    { label: "Active Products", value: safeProducts.length },
    ...
    <main className="grid min-h-screen grid-cols-[250px_1fr] bg-[#edf1f6]">
      <aside className="flex flex-col border-r border-slate-200 bg-white p-5">
        ...
        <div className="mt-6 space-y-2">
          {["overview", "projects", "products", "clients", "settings"].map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
      ...
      <section className="p-7">
        <div className="mb-4 flex items-center justify-between rounded-md border border-slate-200 bg-white px-4 py-3">
          <div className="flex items-center gap-6 text-sm text-slate-700">
            <button onClick={() => navigate("/")} className="hover:text-[#1f3f99]">
              Home
            </button>
            <button
              onClick={() => navigate("/projects")}
              className="hover:text-[#1f3f99]"
            >
              Projects
            </button>
            <button
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <Card className="rounded-md p-5">
                ...
                <div className="mt-4 space-y-3">
                  <Button
                    className="w-full bg-[#1f3f99] text-white"
        ...
        {tab === "projects" && (
          <Card className="overflow-hidden rounded-md">
            <div className="flex items-center justify-between border-b border-slate-200 p-4">
              ...
              <Button
                className="bg-[#1f3f99] text-white"
                onClick={() => setProjectModal("new")}
              >
                + New Project
              </Button>
                  ...
                  {filteredProjects.map((p) => (
                    <tr key={p.id} className="border-t border-slate-200">
                      ...
                      <td className="space-x-3">
                        <button
                          className="font-semibold text-[#1f3f99]"
        ...
        {tab === "clients" && (
          <Card className="rounded-md p-10 text-center">
            <div className="mx-auto mb-3 grid h-14 w-14 place-items-center rounded-full bg-slate-100 text-2xl">
              👥
            </div>
            ...
            <p className="mt-2 text-slate-600">
              This feature will allow you to manage client information, track
        ...
        {tab === "settings" && (
          <Card className="rounded-md p-5">
            <div className="mb-4 flex gap-3 text-sm">
              <span className="rounded-md bg-[#1f3f99] px-3 py-1 text-white">
                General
              </span>
